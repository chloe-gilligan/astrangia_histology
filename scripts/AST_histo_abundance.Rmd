...
##Trying out stats for biostats project
```{r}
...
#Doing stats for tanks and experimental samples
...
#Stats done here for now


```{r}
df_avg <- df %>%
  mutate(
    total = StageI + StageII + StageIII + StageIV + StageV,
    avg_stage = (StageI*1 + StageII*2 + StageIII*3 + StageIV*4 + StageV*5) / total
  )
```
...
data <- read_csv("../data/astrangia_histology_abundance.csv", na = c("", "NA")) %>%
  select(full_sample_id, timepoint, treatment, date, species, month, site, sex,
         StageI, StageII, StageIII, StageIV, StageV) %>%
  mutate(
    full_sample_id = ifelse(startsWith(full_sample_id, "AST"),
                            gsub("-TP\\d+", "", full_sample_id),
                            full_sample_id),
  ...
  filter(timepoint != "TP0")
#Filtering out TP0 to figure out impacts post beginning of expirment
...
model_all_stages <- lmer(
  value ~ stage * treatment * timepoint * sex, 
  data = Rel.Abundance_long
)

summary(model_all_stages)
...
# Simplified glm?
```{r}
glmmTMB(value ~ stage * treatment * timepoint + sex, family = beta_family(), data = Rel.Abundance_long)
...
model <- lmer(response ~ fixed_effects + (random_effects | grouping_factor), data = your_data)
...
# Remove zeros and ones if necessary (beta regression doesn't like exact 0 or 1)
Rel.Abundance_long <- Rel.Abundance_long %>%
  filter(value > 0 & value < 1)

# Fit the model
mod_all_stages <- glmmTMB(
  value ~ stage * treatment * timepoint * sex,
  data = Rel.Abundance_long,
...
data <- read_csv("astrangia_histology_abundance.csv")
...
glm_stage1 <- glm(Count ~ treatment * sex, 
                  data = filter(data_long, Stage == "StageI"), 
                  family = poisson)
summary(glm_stage1)

library(lme4)
mixed_model <- glmer(Count ~ treatment * sex * Stage + (1|full_sample_id), 
...
# Load and clean data (modified from your code)
data <- read_csv("astrangia_histology_abundance.csv", 
                na = c("", "NA")) %>%
  select(full_sample_id, timepoint, treatment, date, species, 
         month, site, sex, StageI, StageII, StageIII, StageIV, StageV) %>%
  filter(!is.na(sex), !is.na(treatment)) %>%
  mutate(sex = factor(sex),
         treatment = factor(treatment),
         month = recode(month, "Juy" = "July"))
...
# Mixed effects model (accounts for repeated measures)
stage_model <- glmer(Count ~ treatment * sex * Stage + (1|full_sample_id),
                    family = poisson,
                    data = long_data)

# Model summary and significance
summary(stage_model)
car::Anova(stage_model, type = 3)
