...
```{r}
Rel.Abundance <- data %>%
  group_by(sex, timepoint, treatment) %>%
  summarise(
...
#separate dataset to experimetnal and field
```{r}

Rel.Abundance_long_field <- Rel.Abundance_long %>%
  filter(treatment=="Field")

ggplot(Rel.Abundance_long_field, aes(x = timepoint, y = value, fill = stage)) +
...
ggplot(Rel.Abundance_long_tank, aes(x = timepoint, y = value, fill = stage)) +
  geom_bar(stat = "identity", position = "fill") +
...
```{r}
ggplot(Rel.Abundance_long, aes(x = timepoint, y = value, fill = stage)) +
  geom_bar(stat = "identity", position = "fill") +
  facet_grid(sex ~ treatment) +
  theme_minimal() +
  labs(
